...
class DataProcessor:
    def __init__(self,input_file,output_dir):
        ...
        ## Creating output directory for data processing
        os.makedirs(self.output_dir,exist_ok=True)
        logger.info(f"Output directory created at {self.output_dir}")
        logger.info("Data processing intialized")

    # load the data
    def load_data(self,use_cols):
        try:
            self.rating_df = pd.read_csv(self.input_file, low_memory=True, usecols=use_cols)
    ...
    def filter_users(self,min_rating=400):
        try:
            ...
            logger.info("Filteres users sucessfully....")
    ...
    def encode_data(self):
        try:
            ...
            logger.info("Encoded data successfully for users and anime..... ")
    ...
    def split_data(self,test_size=1000, random_state=43):
        try:
            ...
            self.y_test = y_test
            logger.info("Data split successfully into train and test set")
    ...
    def save_artifacts(self):
        try:
            ...
            for name,data in artifacts.items():
                ...
                logger.info(f"Saved {name} artifacts successfully")
    ...
    def process_anime_data(self):
        try:
            logger.info(f"Reading anime data from {ANIME_CSV}")
            df = pd.read_csv(ANIME_CSV)
            logger.info(f"Reading anime synopsis from {ANIME_SYNOPSIS_CSV}")
            ...
            logger.info(f"Anime data saved to {DF}")
            synopsis_df.to_csv(SYNOPSIS_DF,index=False)
            logger.info(f"Anime synopsis data saved to {SYNOPSIS_DF}")
    ...
    def run(self):
        try:
            self.load_data(use_cols=['user_id','anime_id','rating'])
            self.filter_users()
            self.scale_ratings()
            self.encode_data()
            self.split_data()
